...
namespace CFPABot.Command
{
    public sealed class GitRepoManager : IDisposable
    {
        ...
        {
            var referencePath = RepoCacheManager.EnsureReferenceCache(repoOwner, repoName);
            Run($"clone --reference-if-able \"{referencePath}\" {(noDepth ? "" : "--depth=3")} -b {branchName} https://x-access-token:{GitHub.GetToken()}@github.com/{repoOwner}/{repoName}.git .");
