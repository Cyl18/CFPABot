...
namespace CFPABot.Utils
{
    public class ModKeyAnalyzer
    {
            ...
            StringBuilder reportSb, MCVersion mcVersion, string downloadModName, string enfile, string cnfile)
        {
            ...
            sb.AppendLine($"⚠ 警告：PR {modid}-{mcVersion.ToVersionString()} 中所提供的英文语言文件与最新模组 `{downloadModName}` 内的英文语言文件不对应。这可能是由于机器人自动获取的模组不是最新，语言文件中包含扩展模组，或所提交的语言文件来自模组源代码仓库。");

            if (enExcept.Count > 0)
            {
                sb.AppendLine($"- 英文语言文件有 {enExcept.Count} 个 Key 多于模组内语言文件。例如 {enExcept.Take(3).Select(f => $"行 {(enfile.Split('\n').ToList().FindIndex(l => l.Contains(f)) + 1)}-`{f}`").Connect(" ")}。");
                reportSb.AppendLine($"英文多于模组内的 Key: \n{enExcept.Select(k => $"    {k}\n").Connect("")}");
            }

            if (cnExcept.Count > 0)
            {
                sb.AppendLine($"- 模组内语言文件有 {cnExcept.Count} 个 Key 多于英文语言文件。例如 {cnExcept.Take(3).Select(f => $"行 {(cnfile.Split('\n').ToList().FindIndex(l => l.Contains(f)) + 1)}-`{f}`").Connect(" ")}。");
