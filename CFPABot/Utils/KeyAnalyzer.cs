...
namespace CFPABot.Utils
{
    public class KeyAnalyzer
    {
            ...
            StringBuilder reportSb, MCVersion mcVersion, string enfile, string cnfile)
        {
            ...
            if (enExcept.Count > 0)
            {
                sb.AppendLine($"- 英文有 {enExcept.Count} 个 Key 多于中文。例如：\n{enExcept.Take(3).Select(f => $"  - 行 {(enfile.Split('\n').ToList().FindIndex(l => l.Contains(f)) + 1)}-`{f}`").Connect("\n")}");
                reportSb.AppendLine($"英文多于中文的 Key: \n{enExcept.Select(k => $"    {k}\n").Connect("")}");
            }

            if (cnExcept.Count > 0)
            {
                sb.AppendLine($"- 中文有 {cnExcept.Count} 个 Key 多于英文。例如：\n{cnExcept.Take(3).Select(f => $"  - 行 {(cnfile.Split('\n').ToList().FindIndex(l => l.Contains(f)) + 1)}-`{f}`").Connect("\n")}");
