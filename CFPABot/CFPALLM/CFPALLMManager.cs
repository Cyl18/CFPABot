...
public static class CFPALLMManager
{
    public static async Task<(PRReviewAssistantData[] data, string rawOutput)> RunPRReview(int prid, string path)
    {
        ...
        {
            new Message(Role.User, "请你扮演一位Minecraft模组语言翻译PR审核者，根据以下Json Schema审核此PR，如果你觉得什么都很好则输出空数组，注意错别字，漏翻，意思不对的情况，你需要提出改进方案（使用```suggestion来表示哪处要换成什么并加上说明和英文原文），```suggestion的前面和后面都需要换行；如果不确定，则提出疑问；在使用```suggestion的时候要注意只修改键值对的值，特别要注意保留空格和键；" +
                                   "你可以对连续的行进行建议\n你可以以<think>进行思考，你必须以下方的Json Schema来输出内容，如果不是回复，InReplyToId则为-1，如果有回复，InReplyToId则为回复的ID；ReplyContent则为你的建议，例如输出" + """[{"Key":"a.b","InReplyToId":-1,"ReplyContent":"我的建议是:___"}]。不仅是对其他人的回复进行回复，你也可以提交新的建议。""" + "\n" +
